...
def split_optimizer(model: nn.Module, cfg: dict):
    ...
    if cfg['optimizer'] == 'Adam':
        optimizer = Adam(param_other, lr=cfg['lr'], betas=(cfg['momentum'], 0.999))  # adjust beta1 to momentum
    ...
    optimizer.add_param_group({'params': param_weight_decay, 'weight_decay': cfg['weight_decay']})  # add pg1 with weight_decay
    optimizer.add_param_group({'params': param_bias})   # add pg2 (biases)
    return optimizer



def cosine_lr_scheduler(optimizer, epochs, final_ratio=0.1):
    l_f = lambda x: (((1 + math.cos(x * math.pi / epochs)) / 2) ** 1.0) * (1-final_ratio) + final_ratio
...
class WarmUpCosineDecayLRAdjust(object):
    ...
    def __call__(self, optimizer, iter, epoch):
        # lr_schedular 返回的是学习率变化所乘以的权重因子
